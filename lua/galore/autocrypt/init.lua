...
function M.update_gossip(message)
  ...
  -- XXX flags are wrong?
  local ahlist =
    gp.message_get_autocrypt_gossip_headers(message, nil, config.values.decrypt_flags, nil)
...
--- Import all gossip
--- In an email
function M.setup_import(message, password) end

local function expired(keydata) end
...
local function show_acc(buffer, accounts)
  ...
  for _, acc in ipairs(accounts) do
    ...
    table.insert(
      box,
      string.format(
        'Addr: %s, enabled: %s, prefer: %s',
        acc.addr,
        show_bool(acc.enabled),
        show_bool(acc.prefer)
      )
    )
  end
  ...
  buffer.create({
    ...
    init = function(buffer)
      ...
      buffer:setlines(
        ...
        0,
        true,
        { string.format('You have %d of accounts with %d peers', #accounts, #peers) }
      )
    end,
  })
end
